...
export async function POST(request: NextRequest) {
  try {
    ...
    const { action, chunks } = await request.json()
    ...
    // Execute Instagram schema migration
    if (action === 'run_instagram_migration') {
      const results = []
    ...
    return NextResponse.json({
      ...
      available_actions: ['init', 'run_instagram_migration', 'status']
    }, { status: 400 })
