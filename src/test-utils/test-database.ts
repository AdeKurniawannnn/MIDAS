...
export class TestTransaction {
  ...
  static async rollback() {
    ...
    try {
      ...
      throw new Error(`Failed to rollback transaction: ${(error as Error).message}`)
    }
  }
  ...
  static async cleanupAllTestData() {
    ...
    try {
      ...
      const cleanupOperations = [
        ...
        () => testSupabase.from('instagram_post_hashtags').delete().gte('created_at', new Date(Date.now() - 86400000).toISOString()),
        () => testSupabase.from('instagram_post_mentions').delete().gte('created_at', new Date(Date.now() - 86400000).toISOString()),
...
// Note: TestValidators class commented out to avoid Jest dependencies in production builds
// Use this class in test files only by uncommenting when needed
/*
    ...
    if (data.length > 0) {
      data.forEach((item: any, index: number) => {
        try {
          ...
          throw new Error(
            `Validation failed for ${entityType} at index ${index}: ${(validationError as Error).message}`
          )
        }
      })
    }
...
export const TestValidators = {
  validateKeyword: () => console.log('TestValidators commented out for production builds'),
  validateGoogleMapsPlace: () => console.log('TestValidators commented out for production builds'),
  validateScrapingJob: () => console.log('TestValidators commented out for production builds'),
  validateAssignment: () => console.log('TestValidators commented out for production builds'),
  validateInstagramHashtag: () => console.log('TestValidators commented out for production builds'),
  validateInstagramPost: () => console.log('TestValidators commented out for production builds'),
  validateTestDataIntegrity: () => console.log('TestValidators commented out for production builds'),
}
...
export class TestDatabaseHealth {
  static async checkConnection() {
    try {
      ...
      if (error && !errorMessage.includes('relation') && !errorMessage.includes('does not exist')) {
      ...
      const counts = await Promise.all(
        tables.map(async table => {
          try {
            ...
            return { table, count: 0, error: (error as Error).message }
          }
        })
      )
