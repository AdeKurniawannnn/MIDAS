#!/usr/bin/env node
/**
 * Complete Instagram Migration - Direct Database Execution
 * This script completes the Instagram schema migration by executing
 * the remaining SQL directly via database connection using pg library.
 */
...
try {
  ...
  console.error('❌ PostgreSQL client (pg) not available.')
...
const dbConfig = {
  host: process.env.PGHOST || '217.15.164.63',
  port: process.env.PGPORT || 5434,
  user: process.env.PGUSER || 'postgres',
  password: process.env.PGPASS,
  database: process.env.PGDATABASE || 'postgres',
  ssl: false, // Direct connection, no SSL needed
  max: 1, // Single connection for migration
  idleTimeoutMillis: 30000,
  connectionTimeoutMillis: 5000,
}

console.log('🚀 Completing Instagram Schema Migration via Direct DB Connection')
console.log('Host:', dbConfig.host)
console.log('Port:', dbConfig.port)
console.log('Database:', dbConfig.database)
console.log('User:', dbConfig.user)
console.log('Password:', dbConfig.password ? 'Present' : 'Missing')

if (!dbConfig.password) {
  console.error('❌ Missing database password (PGPASS)')
  process.exit(1)
}

// SQL to complete the migration
const completionSQL = `
-- =============================================
...
const executeMigration = async () => {
  ...
  try {
    ...
    console.log('\n🔧 Executing completion SQL...')
    
    // Execute the completion SQL
    ...
    `)
    console.log(`📊 Found ${indexesResult.rows.length} Instagram indexes`)
    ...
    `)
    console.log(`⚙️  Found ${functionsResult.rows.length} Instagram functions`)
    ...
    const viewsResult = await client.query(`
      ...
      WHERE table_name IN ('hashtag_analytics', 'user_post_analytics') 
        ...
        AND table_type = 'VIEW'
    `)
    console.log(`👁️  Found ${viewsResult.rows.length} Instagram views`)
...
// Create the large insert function separately (due to size)
...
`
  ...
  try {
    ...
    await client.query(insertFunctionSQL)
...
const main = async () => {
  console.log('\n' + '='.repeat(80))
  console.log('🔧 COMPLETING INSTAGRAM SCHEMA MIGRATION VIA DIRECT CONNECTION')
  ...
  try {
    ...
    if (migrationSuccess) {
      ...
      console.log('\n' + '='.repeat(80))
      console.log('🎉 MIGRATION COMPLETION SUCCESSFUL!')
      ...
      console.log('   node scripts/validate-instagram-schema.js')
      console.log('='.repeat(80))
    } else {
...
// Check if pg is available and run
if (pg) {
