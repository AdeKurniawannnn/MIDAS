...
async function executeSQL(query) {
  try {
    // Execute SQL using fetch to PostgREST raw SQL endpoint
...
async function logMigration(chunk, success, executionTime, errorMessage = null) {
  try {
...
async function runMigration() {
  console.log('🚀 Starting Instagram Schema Migration...')
  ...
  for (const chunk of MIGRATION_CHUNKS) {
    ...
    try {
      ...
      console.log(`✅ Success: ${chunk.name} (${executionTime}ms)`)
      
    } catch (error) {
      const executionTime = Date.now() - startTime
      
      await logMigration(chunk, false, executionTime, error.message)
      
      console.error(`❌ Failed: ${chunk.name}`)
  ...
  if (success) {
    ...
    console.log(`📊 Chunks executed: ${results.length}/${MIGRATION_CHUNKS.length}`)
    ...
    console.log(`❌ Failed at: ${results.length + 1}/${MIGRATION_CHUNKS.length}`)
    process.exit(1)
  }
}
...
module.exports = { runMigration, MIGRATION_CHUNKS }